...
>
> **Stacks testadas:** Rust + Axum (sqlx), Go + Gin (GORM), Spring WebFlux (Kotlin, R2DBC), Spring MVC (Kotlin, JDBC), FastAPI Async (Python, SQLAlchemy)
...
| 2 | Go + Gin (GORM) | 10.958 req/s | 17,5ms | 43,3ms | 98ms | 0,00% | 2,8x |
| 3 | Spring WebFlux | 9.641 req/s | 14,1ms | 68,0ms | 109ms | 0,23% | 19,0x |
| 4 | Spring MVC | 3.046 req/s | 57,8ms | 185,0ms | 524ms | 0,00% | 7,0x |
| 5 | FastAPI Async (SQLAlchemy) | 1.584 req/s | 67,1ms | 503,5ms | 3.203ms | 0,00% | 3,0x |
...
| Métrica | Rust + Axum | Go + Gin (GORM) | WebFlux | MVC | FastAPI |
|---|---|---|---|---|---|
| Throughput | **4.102/s** | 2.767/s | 147/s | 113/s | 2.500/s |
| p95 | **85ms** | 259ms | 3.631ms | 6.997ms | 493ms |
| Erros | **0%** | **0%** | **0%** | **0%** | **87%** |
...
| Métrica | Rust + Axum | Go + Gin (GORM) | WebFlux | MVC | FastAPI |
|---|---|---|---|---|---|
| Throughput | **12.228/s** | 8.234/s | 669/s | 896/s | 1.798/s |
| p95 | **26ms** | 83ms | 896ms | 1.206ms | 489ms |
| Erros | **0%** | **0%** | **0%** | **68%** | **57%** |
...
| Métrica | Rust + Axum | Go + Gin (GORM) | WebFlux | MVC | FastAPI |
|---|---|---|---|---|---|
| Throughput | **14.906/s** | 12.926/s | 1.932/s | 1.421/s | 2.478/s |
| p95 | **15ms** | 36ms | 226ms | 432ms | 451ms |
| Erros | **0%** | **0%** | **0%** | **0%** | **53%** |
...
| Métrica | Rust + Axum | Go + Gin (GORM) | WebFlux | MVC | FastAPI |
|---|---|---|---|---|---|
| Throughput | **15.332/s** | 11.281/s | 4.235/s | 2.464/s | 2.287/s |
| p95 | **15ms** | 23ms | 104ms | 219ms | 324ms |
| Erros | **0%** | **0%** | **0%** | **0%** | **43%** |
...
| Métrica | Rust + Axum | Go + Gin (GORM) | WebFlux | MVC | FastAPI |
|---|---|---|---|---|---|
| Throughput | **15.076/s** | 11.137/s | 6.507/s | 2.580/s | 1.529/s |
| p95 | **15ms** | 21ms | 71ms | 218ms | 327ms |
| Erros | **0%** | **0%** | **0%** | **0%** | **0%** |
...
| CPU | Rust + Axum | Go + Gin (GORM) | WebFlux | MVC | FastAPI |
|---|---|---|---|---|---|
| 1 | **7.032/s** (94ms p95) | 3.959/s (221ms) | 507/s (1.113ms) | 432/s (1.497ms) | 520/s (1.481ms) |
| 2 | **11.860/s** (55ms p95) | 8.038/s (114ms) | 2.094/s (267ms) | 1.185/s (510ms) | 1.055/s (720ms) |
| 4 | **14.421/s** (33ms p95) | 10.118/s (50ms) | 4.746/s (141ms) | 3.047/s (187ms) | 1.606/s (463ms) |
| 8 | **14.762/s** (32ms p95) | 10.921/s (44ms) | 7.629/s (88ms) | 3.062/s (190ms) | 1.626/s (455ms) |
| 12 | **14.578/s** (32ms p95) | 10.958/s (43ms) | 9.641/s (68ms) | 3.046/s (185ms) | 1.584/s (503ms) |
...
| CPU | Rust + Axum | Go + Gin (GORM) | WebFlux | MVC | FastAPI |
|---|---|---|---|---|---|
| 1 | **2.645/s** (63ms p95, **0% err**) | 1.910/s (203ms, **0% err**) | 1.135/s (614ms, 0,20% err) | 752/s (712ms, 0,83% err) | 454/s (1.398ms, 0,22% err) |
| 2 | **3.570/s** (15ms p95, **0% err**) | 2.771/s (71ms, **0% err**) | 1.631/s (223ms, 0% err) | 1.659/s (201ms, 0% err) | 938/s (657ms, 0,03% err) |
| 4 | **3.759/s** (8ms p95, **0% err**) | 3.331/s (25ms, **0% err**) | 2.137/s (111ms, 0% err) | 1.644/s (168ms, 0% err) | 1.224/s (414ms, 0% err) |
| 8 | **3.778/s** (7ms p95, **0% err**) | 3.390/s (16ms, **0% err**) | 2.792/s (49ms, 0% err) | 1.643/s (168ms, 0% err) | 1.237/s (410ms, 0% err) |
| 12 | **3.777/s** (7ms p95, **0% err**) | 3.379/s (17ms, **0% err**) | 3.171/s (28ms, 0% err) | 1.644/s (166ms, 0% err) | 1.211/s (420ms, 0% err) |
...
- Segundo lugar geral com margem confortável sobre as stacks JVM
...
### 🥉 Spring WebFlux — A melhor stack JVM
...
### 4º Spring MVC — O clássico limitado
...
### 5º FastAPI Async (SQLAlchemy) — O mais fraco em throughput
...
> **Rust Axum com 2 CPUs (11.860 req/s) entrega mais throughput que QUALQUER stack JVM com 12 CPUs.** Go Gin com 2 CPUs (8.038/s) também supera o pico do WebFlux (9.641/s com 12 CPUs).
>
