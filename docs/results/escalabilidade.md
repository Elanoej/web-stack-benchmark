...
- **1 CPU / 1GB**: stacks com 0% erro → Go + Gin (GORM), Rust + Axum (sqlx), Spring WebFlux, Spring MVC
- **2 CPUs / 2GB**: stacks com 0% erro → Go + Gin (GORM), Rust + Axum (sqlx), Spring WebFlux
- **4 CPUs / 4GB**: stacks com 0% erro → Go + Gin (GORM), Rust + Axum (sqlx), Spring WebFlux, Spring MVC
...
| 5 | FastAPI Async (SQLAlchemy) | 1584 req/s | 503 ms | 0.00% | 3.0x |
...
2. **Go + Gin (GORM)** mantém a segunda posição com vantagem sobre as stacks JVM. Sua eficiência por núcleo é notável: com 2 CPUs já supera o throughput máximo de todas as stacks concorrentes exceto Rust.
3. **Spring WebFlux** é a melhor stack JVM, com escalabilidade consistente (19x de 1 para 12 CPUs em ramp-up) e 0% de erro em steady state em todas as configs.
4. **Spring MVC** sofre com o modelo thread-per-request. Escala bem até 4 CPUs e então platôa — o bottleneck muda para o banco ou nginx.
5. **FastAPI Async (SQLAlchemy + Pydantic)** é a stack mais fraca em throughput bruto. O overhead do ORM + validação + GIL limita o throughput máximo.
